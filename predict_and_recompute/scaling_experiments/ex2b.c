...
int main(int argc,char **args)
{
  ...
  PetscReal      norm,vOff=1e-3,vDiag,rho=0.8,kappa=1e3;     /* norm of solution error */
  PetscInt       i,j,Ii,J,Istart,Iend,n = 8,k = 2,K,its,repeat,numRepeat=1;
  PetscErrorCode ierr;
  PetscBool      flg;
//  PetscScalar    v;
  ...
  ierr = PetscOptionsGetInt(NULL,NULL,"-num_repeat",&numRepeat,NULL);CHKERRQ(ierr);
  ...
  ierr = PetscOptionsGetScalar(NULL,NULL,"-off_value",&vOff,NULL);CHKERRQ(ierr);
  ...
  */
  ...
  for (Ii=Istart; Ii<Iend; Ii++) {
   ...
   for (j=Ii-k; j<=Ii+k; j++) {
      // small off diagonals
      if ( j>=0 && j<n && Ii != j) {
        MatSetValues(A,1,&Ii,1,&j,&vOff,ADD_VALUES);
      }
    }
   
    vDiag = 1.0 + ( (1.0*Ii) / (1.0*(n-1)) ) * (kappa - 1) * PetscPowReal(rho,n-1-Ii);
    ierr = MatSetValues(A,1,&Ii,1,&Ii,&vDiag,ADD_VALUES);CHKERRQ(ierr);
  }
  ...
  */
  ierr = KSPSetTolerances(ksp,0,0,PETSC_DEFAULT,PETSC_DEFAULT);CHKERRQ(ierr);
