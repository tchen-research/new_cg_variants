...
int main(int argc,char **args)
{
  ...
  PetscInt       j,Ii,Istart,Iend,n = 10,k = 3,K,its;
  PetscErrorCode ierr;
  PetscBool      flg;
  PetscScalar    kappa = 1e3,rho = 0.8,offValue = 1e-2,diagValue;
  PetscLogStage  stage;
  ...
  ierr = PetscOptionsGetScalar(NULL,NULL,"-off_value",&offValue,NULL);CHKERRQ(ierr);
  ...
  */
  ...
  ierr = MatMPIAIJSetPreallocation(A,K,NULL,K,NULL);CHKERRQ(ierr);
  ierr = MatSeqAIJSetPreallocation(A,K,NULL);CHKERRQ(ierr);
  ierr = MatSeqSBAIJSetPreallocation(A,1,K,NULL);CHKERRQ(ierr);
  ierr = MatMPISBAIJSetPreallocation(A,1,K,NULL,K,NULL);CHKERRQ(ierr);
  ierr = MatMPISELLSetPreallocation(A,K,NULL,K,NULL);CHKERRQ(ierr);
  ierr = MatSeqSELLSetPreallocation(A,K,NULL);CHKERRQ(ierr);
  ...
  */
  ...
  /*
     Set matrix elements for approximate k-banded model problem
  ...
  for (Ii=Istart; Ii<Iend; Ii++) {
    ...
    // set diagonals to eigenvalues of model problem
    diagValue = 1.0 + ( (1.0*Ii) / (1.0*(n-1)) ) * (kappa - 1) * PetscPowReal(rho,n-1-Ii);
    ierr = MatSetValues(A,1,&Ii,1,&Ii,&diagValue,INSERT_VALUES);CHKERRQ(ierr);
    //printf("diagonal Value %i: %f",Ii,diagValue);

    // set off diagonals to be small constant    
    for (j=0; j<n; j++) {
      // small off diagonals
      if ( Ii < j+k && Ii > j-k && Ii != j) {
        ierr = MatSetValues(A,1,&Ii,1,&j,&offValue,INSERT_VALUES);CHKERRQ(ierr);
  ...
  */
  ierr = KSPSetTolerances(ksp,0.0,0.0,PETSC_DEFAULT,PETSC_DEFAULT);CHKERRQ(ierr);
