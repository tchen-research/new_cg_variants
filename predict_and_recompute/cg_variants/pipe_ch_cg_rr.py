...
def pipe_ch_cg_rr(A,b,x0,max_iter,variant='',rk_replace=lambda k:False,callbacks=[],**kwargs):
    '''
    ...
    # initialize
    output = {}
    output['name'] = f"pipe_ch_cg{'_'+variant if variant!= '' else''}_rr"
...
def pipe_ch_cg_b_rr(*args,**kwargs):
    return pipe_ch_cg_rr(*args,variant='b',**kwargs)


def pipe_ch_pcg_rr(A,b,x0,max_iter,variant='',preconditioner=lambda x:x,rk_replace=lambda k:False,callbacks=[],**kwargs):
    '''
    ...
    # initialize
    output = {}
    output['name'] = f"pipe_ch_pcg{'_'+variant if variant!= '' else''}_r"
    output['max_iter'] = max_iter
...
def pipe_ch_pcg_b_rr(*args,**kwargs):
    return pipe_ch_pcg_rr(*args,variant='b',**kwargs)
